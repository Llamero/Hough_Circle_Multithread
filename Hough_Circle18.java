...
IJ.log("Total time: " + totalTime + ", Time per circle: " + totalTime/nCircles);
    ...
    private void convertLocaltoFullHough(int slice, ImageStack houghStack){
        ...
        //Fill the Hough space with the local transforms
            ...
            for(int h=0; h<lheight; h++){    
                for(int w=0; w<lwidth; w++){
                    for(int i=0; i<ldepth; i++){
                       localHoughPixels[i+(lradiusMin-radiusMin)/radiusInc][(w+startWidth)+(h+startHeight)*width] = (byte) Math.round ((localHoughValues[circle][w][h][i] * 255D) / maxHough);
                    }
                }
            }
        }
