...
public class Hough_Circle implements PlugInFilter {
    ...
    private double maxHough; //Contains the brights pixel in the entire Hough array
    private Point maxPoint; //Stores the location of the brightest pixel in the Hough array
    private double maxRadius; //Stores the radius of the brightest pixel in the Hough array
    ...
    private double maxHoughArray[][]; //Matrix to store hough scores, radii, and points from multi-threaded max search
    private int ithread;
    // </editor-fold>
    ...
    private void houghMaximum () {
        ...
        maxHoughArray = new double[threads.length][4];
        ...
        for (ithread = 0; ithread < threads.length; ithread++) {    
            ...
            threads[ithread] = new Thread() {  
                ...
                @Override
                public void run() {
                    double maxHoughThread = 0D;
                    double maxRadiusThread = 0D;
                    ...
                    maxHoughArray[Index.getAndIncrement()] = new double[]{maxHoughThread, maxRadiusThread, maxPointThread.x, maxPointThread.y};                    
                }
            };
        }
        startAndJoin(threads);
        ...
        for (double[] maxHoughArray1 : maxHoughArray) {
