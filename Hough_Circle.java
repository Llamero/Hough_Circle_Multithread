...
public class Hough_Circle implements PlugInFilter {
    ...
    private int houghIndexCount; //Contains the total number of radii tested
    ...
    private void startTransform(){
        ...
        if(houghSeries){
            ...
            //IJ.log("" + houghPlus.getStackSize());
        }
        if(showCircles){
            circlePlus = imp.createImagePlus();
            circleStack = imp.createEmptyStack();
        }
        if(showRadius){
            radiusPlus = imp.createImagePlus();
            radiusStack = imp.createEmptyStack();
        }
        if(showScores){
            scorePlus = imp.createImagePlus();
            scoreStack = imp.createEmptyStack();
        }
        ...
        for(int slice = 1; slice <= imp.getStackSize(); slice++){
            ...
            if(houghSeries){
             ...
             if(showCircles){
                 new ImagePlus(nCircles+" Circles Found", circlesip).show();
             }
         ...
         //Remove the ImagePlus from memory as it is no longer needed
         //imp.flush(); //Do not do this as it will erase original data
